    ...
    <div className="w-full bg-white rounded-lg shadow-sm text-sm p-4 items-start">
      ...
      <div className="flex-1">
        <h2 className="text-base font-semibold mb-2 flex items-center gap-2">
          {port.name}
          <Anchor size={18} className="text-red-400" strokeWidth={2.25} />{' '}
        </h2>
        <div>
          <p>
          ...
          <p>
            <strong>Max Length:</strong> {port.max_length_m} m
          </p>
          <p>
            <strong>Berths:</strong> {port.berths}
          </p>
          ...
          <ul className="list-disc list-inside max-h-40 overflow-auto text-xs">
            {port.last_visited_ships.map(port => (
              <li key={port.imo}>
                {port.name} — TEU: {port.teu}, LOA: {port.loa_m}m, Visited:{' '}
                {new Date(port.visited_at).toLocaleDateString()}
              </li>
            ))}
          </ul>
        </div>
